## HIPOTESE 3 - MEDV X PTRATIO
# Imóveis em regiões com maior razão de alunos por professor (PTRATIO) tendem a ter menor valor de mercado (MEDV).
...
corr_pearson, p_value = pearsonr(df['PTRATIO'], df['MEDV'])
...
print(f"\n=== ANÁLISE DE CORRELAÇÃO: PTRATIO vs MEDV ===")
print(f"VALOR: r = {corr_pearson:.4f}")
print(f"DIREÇÃO: {direcao.capitalize()}")
print(f"FORÇA: {forca.capitalize()}")
print(f"INTERPRETAÇÃO: Correlação {direcao} {forca} entre razão aluno-professor e valor dos imóveis")
print(f"="*60)
...
plt.xlabel('Razão Aluno-Professor (PTRATIO)')
...
print(f"H₀: ρ = 0 (Não há correlação linear entre PTRATIO e MEDV)")
print(f"H₁: ρ ≠ 0 (Há correlação linear entre PTRATIO e MEDV)")
...
if p_value < alpha:
    ...
    print(f"CONCLUSÃO: Há evidências estatisticamente significativas de correlação {direcao} {forca} entre PTRATIO e MEDV")
else:
    print(f"DECISÃO: Como p-valor ({p_value:.6f}) ≥ α ({alpha}), não rejeitamos H₀")
    print(f"CONCLUSÃO: Não há evidências estatisticamente significativas de correlação linear entre PTRATIO e MEDV")